...
#SBATCH --array=1-3000 #number of jobs to run, it is currently set to 1 job(1 year), change it to array=1-13 for 13 years(jobs)
#SBATCH --mem=500 #memory requested
#SBATCH -J ipw_bias_mse_cover_kang  #job name, this case:aw-area weighted aggregation
#SBATCH --mail-type=ALL #notifications for job done
#SBATCH --mail-user=xiw321@mail.harvard.edu # send to address
...
R CMD BATCH --quiet --no-restore --no-save bias_mse_cover_kang_ipw.R /n/home01/xwu1993/ts_ips2/output/test_${SLURM_ARRAY_TASK_ID}.Rout
